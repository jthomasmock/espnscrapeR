...
get_nfl_boxscore_players <- function(game_id){
  ...
  game_header <- raw_json %>%
    ...
    select(
      !any_of(c(
        "links", "logos", "possession", "broadcasts", "league", "linescores", "record", "status",
        "rank", "team_uid", "team_name", "team_abb", "team_full", "team_color", "team_color_alt",
        "team_logo", "nickname", "id", "uid"
      ))
